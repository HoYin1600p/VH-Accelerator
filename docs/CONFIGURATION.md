...
| `chunkMeshing` | client | `true` | Implemented; replaces vanilla's allocation-heavy section iterator and reuses the first BlockState lookup only when the position still matches. Automatically unavailable with Fluidlogged. |
| `bufferBuilderLeakFix` | client | `true` | Implemented; prevents a later duplicate RenderType registration from allocating and abandoning a second native BufferBuilder. Automatically unavailable with the two upstream-incompatible render mods. |
...
| `parallelAtlasStitching` | `true` | Prepares independent atlases in bounded batches and routes vanilla texture metadata/PNG child jobs through a dedicated CPU-scaled pool to prevent nested resource-reload starvation; unsafe model graphs retain the original atlas loop. |
